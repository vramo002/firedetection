...
img = cv2.imread("frame6006.jpg", cv2.COLOR_BGR2GRAY)
...
plt.title('Intensity')
ax.add_patch(Rectangle((200, 1), 55, 250, edgecolor='red', fill=False))
plt.show()
adding_200_255 = 0
print(histr[200][0])
print(len(histr))
...
print("Sum of Pixel", adding_200_255)
...
while centroids_index != len(centroids):
    ...
    #new_l = np.convolve(new_l, g, 'valid')
    #new_l = np.convolve(new_l, g, 'valid')
    #new_l = np.convolve(new_l, h, 'valid')
    sumal_noabs = sum(a_l)
    sumdl_noabs = sum(d_l)
    ...
    axs[0].set(xlabel='degrees', ylabel='distance')
